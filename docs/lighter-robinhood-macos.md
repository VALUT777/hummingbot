...
1. Пароль локального хранилища Hummingbot. При первом запуске придумайте новый
   пароль; это не пароль кошелька и не API-ключ. При вставке скрытого значения
   Terminal не показывает даже точки — это нормально.
2. `Account Index` и `API Key Index` из Robinhood Lighter. Для API-ключа допустим
   индекс 4–254; Account Index 0 допустим.
3. `API Private Key` — скрыто, один раз. Префикс `0x` можно оставить. `Public Key`
   мастеру не нужен. Ключ хранится штатным шифрованием Hummingbot и не попадает в
   аргументы процесса, вывод или открытый YAML.
4. Подтверждение `OFF`: переключатель `Maker Only` в интерфейсе генерации ключа
   должен быть выключен. Совместимость этого режима с сервисными операциями не
   подтверждена.
5. Нижнюю и верхнюю цену LIT и явно выбранный резерв USDG. Лимит 1000 LIT,
   плечо 5× и остальные защитные параметры остаются фиксированными.
...
После этого мастер показывает аккаунт, диапазон, лимит, плечо и консервативное
