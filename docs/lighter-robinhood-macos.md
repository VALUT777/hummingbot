...
- Минимальный шаг между уровнями: 0.36%; минимальный размер уровня: 50 USDG.
- Получается примерно 55 уровней, около 54.5 USDG и примерно 10 LIT на уровень
  при цене около 5.5. Реальное число и размер уточняет нативный executor по
  правилам рынка и текущей цене.
...
- Размер пакета и частота также применяются к каждому диапазону отдельно. В
  ...
  интервал — 3 секунды; `activation_bounds: 0.005` (0.5%). Входы дальше 0.5%
  от средней цены снимаются; исполнитель выбирает ближайшие допустимые уровни.
  Этот же штатный порог действует на take-profit: далёкая заявка не выставляется
  до подхода цены к цели, а при удалении от неё снимается. Сетка не переносится
  целиком вслед за рынком. Для изменения параметра нужен перезапуск.
- Вход и take-profit используют `LIMIT_MAKER`; take-profit остаётся 1.8%.
  Stop-loss,
