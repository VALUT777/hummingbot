...
- Upstream v2.17.0 pinned `lighter-sdk==1.0.8` in several dependency sources. The new SDK carries native signers for macOS arm64 and amd64. On this Darwin arm64 Mac, the tested result is a private official-micromamba 2.9.0 environment with Python 3.12.14 under `$HOME/.cache/codex/hummingbot-robinhood-v217-9af100d`; native compilation and `lighter-sdk==1.1.4` imports succeed. The setup did not alter system Python 3.14.6 or shell profiles. Current channel resolution required NumPy 2.2.6, Numba 0.61.2 and cryptography 48.0.1 alignment; `pip check` then passed. Docker/Colima was unnecessary. The setup metadata's lower bound does not prove every dependency supports Python 3.10/3.11.
