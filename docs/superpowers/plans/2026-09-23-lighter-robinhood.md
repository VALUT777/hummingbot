...
- [ ] Inspect `AGENTS.md`, `setup.py`, `setup/environment.yml`, `setup/pip_packages.txt`, build instructions and available runtimes. Use installed uv 0.11.28 to create an isolated Python 3.12 environment on Darwin arm64, subject to actual dependency resolution; system Python 3.14.6 is not the target. Conda/mamba are absent and Docker/Colima is presently stopped. Do not claim the setup.py lower bound proves every dependency supports Python 3.10/3.11.
